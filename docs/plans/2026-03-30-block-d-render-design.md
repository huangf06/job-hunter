...
**v2 (2026-03-30)**: Addressed Codex review ("Request revision", 2 blocking + 7 additional findings):
- **Blocking #1**: Narrowed Change 4 scope — only delete `_validate_tailored_structure()`, explicitly keep `ResumeValidator.validate()` and `_post_render_qa()`
- **Blocking #2**: Added failure policy decision for Change 1 CI step
- Added `json.loads()` guard to Change 6 (adapt path)
- Added `slot_schema` existence guard to Change 6
- Noted `UNIQUE(job_id, role_type)` constraint impact on Change 5
- Removed `adapt_html` from registry schema entirely (not per-template)
- Expanded test plan: +6 tests (invalid JSON, missing schema, c3 FAIL routing, legacy NULL, QA blocking, mixed-tier batch)
- Noted submit-dir race condition as accepted risk
...
用关键词匹配标题返回 `ml_engineer`/`data_engineer`/`quant` 等。功能已被三层路由的 `template_id` (DE/ML/Backend) 取代，但仍有一处活跃调用 (FULL_CUSTOMIZE 主路径 `save_resume()` line 256)。非死代码，但过时。可直接用 `template_id_final` 或 fallback `"general"` 替代。

### 2.5 MEDIUM: `_render_adapt_template()` 缺 JSON 解析防护

**line 325**:
```python
tailored = json.loads(analysis.get('tailored_resume') or '{}')
```
如果 `tailored_resume` 含非法 JSON → `JSONDecodeError` 崩溃，不会优雅返回 None。同时未校验 `slot_schema` 字段存在性。
...
- **删除重复验证，保留防御性验证**：删 `_validate_tailored_structure()` (与 ResumeValidator 重叠)；保留 `ResumeValidator.validate()` (defense-in-depth，CLI 直接调用 `render_resume()` 仍需要) 和 `_post_render_qa()` (检查渲染产物质量，不可被上游替代)
- **每条渲染路径都必须有测试**
- **最小改动**：不改动已验证干净的文件
...
**失败策略决策**: `continue-on-error: true`，generate 失败仅通过 Telegram 通知报告，不导致 workflow 整体失败。理由：
- Generate 依赖 Playwright/Chromium，CI 环境有外部依赖不稳定风险
- Scrape 和 AI 分析的结果已持久化到 DB，generate 失败不会丢失上游工作
- 下次 CI 运行会自动重试 (query 按 `pdf_path IS NULL` 筛选)
- 与现有 scrape/ai-evaluate/ai-tailor 步骤的 `continue-on-error: true` 策略一致
...
**联动**: `config/template_registry.yaml` 中删除所有模板的 `adapt_html` 字段 (DE、ML、Backend)。整个 schema 不再包含此字段——adapt 模板统一为一个文件，不需要 per-template 配置。运行时唯一引用 `adapt_html` 的代码是 `_render_adapt_template()` line 333，本次改动同步修改。
...
**保留** (不动):
- `self.validator.validate()` (line 165) — `ResumeValidator` 的 6 类校验 (bio/title/skills/结构)。保留原因：`render_resume()` 可通过 CLI 直接调用 (`resume_renderer.py --job JOB_ID`)，不经过 ai_analyzer，需要 defense-in-depth。
- `_post_render_qa()` (line 197) — 检查渲染产物质量 (页数估算、空 bullet、双重转义)。这是渲染后校验，不可被任何上游替代。

**理由**: `_validate_tailored_structure()` 是 `ResumeValidator` 的宽松子集 (见 §2.3 对比表)。renderer 内 `ResumeValidator.validate()` 已覆盖相同且更严格的检查，两者串联运行是纯冗余。
...
**DB 约束影响**: `resumes` 表有 `UNIQUE(job_id, role_type)`。改动后 role_type 值域从 `{ml_engineer, data_engineer, quant, data_scientist, general}` 变为 `{DE, ML, Backend, general}`。不会引起冲突——同一 job 不会多次生成不同 tier 的简历。但语义变化会影响历史数据查询 (旧记录用旧值，新记录用新值)。已有 588 条旧记录不做迁移，仅影响新记录。
...
| `test_render_adapt_template_invalid_json_returns_none` | 改动 6: 非法 JSON 优雅失败 |
| `test_render_adapt_template_missing_slot_schema_returns_none` | 改动 6: 缺 slot_schema 优雅失败 |
| `test_render_resume_c3_fail_routes_to_template_copy` | 路由: c3_decision==FAIL 走 template copy (从 render_resume 入口) |
| `test_render_resume_legacy_null_tier_uses_full_customize` | 路由: tier=NULL + template_id_final=NULL 走 legacy 路径 |
...
| `test_post_render_qa_blocking_prevents_save` | 保留验证: QA blocking 阻止 PDF 生成 |
| `test_render_batch_mixed_tiers` | 端到端: batch 含 USE_TEMPLATE + FULL_CUSTOMIZE 混合 |
| `test_render_resume_validator_still_runs` | 保留验证: ResumeValidator.validate() 仍在 renderer 中执行 |
...
| 渲染路径测试覆盖 | 3 tests | 15 tests |
| adapt 模板文件数 | 2 (重复) | 1 |
| CI 能否生成 PDF | 否 | 是 |
| template_id 无效时行为 | KeyError 崩溃 | 优雅返回 None + 日志 |
| adapt 路径非法 JSON | JSONDecodeError 崩溃 | 优雅返回 None + 日志 |
...
- **Submit-dir 竞态**: `_build_output_paths()` 的 sequence 编号 (lines 280-284) 在同 company+date 并发渲染时存在 TOCTOU 竞态。当前单进程运行，风险不存在。如果未来引入并行渲染，需要改用 atomic 方案。不在本次 scope。
- **role_type 语义断裂**: 改动 5 使新记录的 `role_type` 值 (DE/ML/general) 与旧记录 (ml_engineer/data_engineer/...) 不同。不做历史数据迁移，仅影响按 role_type 聚合查询的结果。当前无此类查询。
